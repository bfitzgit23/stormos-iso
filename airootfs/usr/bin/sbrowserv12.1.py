...
class BookmarkSearcher(QDialog):
    ...
    def setup_ui(self):
        ...
        self.search_bar = QLineEdit()
        self.search_bar.setPlaceholderText("Search bookmarks by title, URL, folder or description...")
        self.search_bar.textChanged.connect(self.debounce_search)
        search_layout.addWidget(self.search_bar)
        
        # Search mode selector
        self.search_mode = QComboBox()
        self.search_mode.addItems(["All Fields", "Titles Only", "URLs Only", "Folders Only", "Descriptions Only"])
        self.search_mode.currentIndexChanged.connect(self.perform_search)
    ...
    def load_bookmarks_async(self):
        """Load bookmarks from all sources in a background thread."""
        self.status_bar.showMessage("Loading bookmarks...")
        
        # Create worker thread
        self.bookmark_worker = QThread()
        self.bookmark_worker.start()
        
        # Create worker object
        worker = BookmarkLoader()
        worker.moveToThread(self.bookmark_worker)
        
        # Connect signals
        worker.bookmarks_loaded.connect(self.on_bookmarks_loaded)
        worker.error_occurred.connect(self.on_bookmark_error)
        self.bookmark_worker.started.connect(worker.load_all_bookmarks)
        
        # Clean up when done
        worker.finished.connect(self.bookmark_worker.quit)
        worker.finished.connect(worker.deleteLater)
        self.bookmark_worker.finished.connect(self.bookmark_worker.deleteLater)
    
    def on_bookmarks_loaded(self, bookmarks):
        """Handle loaded bookmarks from background thread."""
        self.all_bookmarks = bookmarks
        self.status_bar.showMessage(f"Loaded {len(bookmarks)} bookmarks", 3000)
        self.perform_search()
    
    def on_bookmark_error(self, message):
        """Handle bookmark loading errors."""
        self.status_bar.showMessage(f"Error: {message}", 5000)
        QMessageBox.warning(self, "Bookmark Error", message)
    
    def debounce_search(self):
        """Debounce search to avoid rapid firing while typing."""
    ...
    def perform_search(self):
        ...
        if not self.all_bookmarks:
            self.results_list.addItem("Loading bookmarks...")
            return
            
        if not query:
            self.results_list.addItem("Enter search terms above")
            return
        ...
        for bookmark in self.all_bookmarks:
            ...
            url_match = (mode in [0, 2]) and search_func(bookmark.get("url", ""))
            folder_match = (mode in [0, 3]) and search_func(bookmark.get("folder", ""))
            desc_match = (mode in [0, 4]) and search_func(bookmark.get("description", ""))
        ...
        if matches:
            ...
            for bookmark in matches:
                item = QListWidgetItem(self.create_bookmark_display(bookmark))
...
class BookmarkLoader(QObject):
    ...
    def parse_chrome_bookmarks(self, data, source):
        """Parse Chrome/Chromium bookmarks JSON with descriptions."""
        bookmarks = []
        
        def traverse(node, folder="Main"):
            if "children" in node:
                current_folder = node.get("name", folder)
                for child in node["children"]:
                    traverse(child, current_folder)
            elif node.get("type") == "url":
                bookmarks.append({
                    "title": node.get("name", ""),
                    "url": node.get("url", ""),
                    "folder": folder,
                    "description": node.get("description", ""),  # Chrome supports descriptions
                    "source": source,
    ...
    def load_firefox_bookmarks(self):
        """Load bookmarks from Firefox with descriptions."""
        ...
        try:
            ...
            with sqlite3.connect(db_copy_path) as conn:
                ...
                cursor.execute("""
                    ...
                    FROM moz_bookmarks
                    JOIN moz_places ON moz_bookmarks.fk = moz_places.id
                    LEFT JOIN moz_items_annos ON moz_bookmarks.id = moz_items_annos.item_id
                    WHERE moz_bookmarks.type = 1
                """)
                ...
                for title, url, parent_id, description in cursor.fetchall():
                    bookmarks.append({
                        "title": title or "",
                        "url": url or "",
                        "folder": folders.get(parent_id, "Other Bookmarks"),
                        "description": description or "",  # Firefox descriptions
                        "source": "Firefox",
                        "date": ""
                    })
            ...
            print("Error loading Firefox bookmarks:", str(e))
            return []
        finally:
            if 'db_copy_path' in locals() and os.path.exists(db_copy_path):
                os.remove(db_copy_path)
    
    def load_local_bookmarks(self):
        """Load bookmarks from local storage with descriptions."""
                    ...
                    for folder, items in data["folders"].items():
                        for item in items:
                            bookmarks.append({
                                ...
                                "url": item.get("url", ""),
                                "folder": folder,
                                "description": item.get("description", ""),  # Local descriptions
                                "source": "Storm Browser",
                                "date": item.get("date", "")
                            })
                    return bookmarks
                return []
        except Exception as e:
            print("Error loading local bookmarks:", str(e))
            return []
...
class BrowserCalendar(QWidget):
    ...
    def setup_ui(self):
        ...
        self.delete_btn.clicked.connect(self.delete_selected_event)  # NEW CONNECTI
    ...
    def delete_selected_event(self):
        ...
        # Extract time from the event text (format: "⏰ HH:mm: Event Name")
        if ": " in selected_text:
...
class BrowserMainWindow(QMainWindow):
    def __init__(self):
        ...
        self._calendar_window = None  # Initialize calendar window reference

        # Setup the interface
        self.setup_ui()
        self.setup_menus()  # Initialize menus before calendar
        self.setup_calendar()  # Now setup calendar after menus exist
...
# Modify the main() function to use StormBrowserDark
    ...
    def setup_menus(self):
        ...
        # View Menu (must be created before calendar setup)
        self.view_menu = menubar.addMenu("View")
        # Other view items will be added here
        
        # Add other menus as needed...
        self.settings_menu = menubar.addMenu("Settings")

    def setup_calendar(self):
        """Initialize calendar widget and dock"""
        ...
        self.calendar_dock.setFeatures(
        ...
        toggle_action = self.calendar_dock.toggleViewAction()
        toggle_action.setText("Calendar Panel")
        toggle_action.setShortcut("Ctrl+Shift+C")
    ...
    def take_screenshot(self, mode="ask"):
        ...
        try:
            ...
            # Capture the browser window instead of the viewport
            ...
            if mode == "clipboard":
                ...
                self._save_screenshot_to_file(screenshot)
            else:  # ask
    ...
    def setup_shortcuts(self):
        ...
        QShortcut(QKeySequence("Esc"), self).activated.connect(lambda: self.current_browser().stop())
        ...
        QShortcut(QKeySequence("Ctrl+Tab"), self).activated.connect(self.focus_next_tab)
        QShortcut(QKeySequence("Ctrl+Shift+Tab"), self).activated.connect(self.focus_prev_tab)
        ...
        QShortcut(QKeySequence("Ctrl+K"), self).activated.connect(self.focus_search_bar)

        # Tools and Features Shortcuts
        QShortcut(QKeySequence("Ctrl+B"), self).activated.connect(self.show_bookmarks)
        QShortcut(QKeySequence("Ctrl+D"), self).activated.connect(self.add_current_to_bookmarks)
        ...
        QShortcut(QKeySequence("Ctrl+P"), self).activated.connect(self.print_current_page)
        QShortcut(QKeySequence("Ctrl+Shift+P"), self).activated.connect(self.print_to_pdf)
        QShortcut(QKeySequence("Ctrl+Shift+S"), self).activated.connect(lambda: self.take_screenshot("ask"))
        QShortcut(QKeySequence("F12"), self).activated.connect(self.toggle_dev_tools)
        QShortcut(QKeySequence("Ctrl+U"), self).activated.connect(lambda: self.current_browser().page().action(QWebEnginePage.ViewSource).trigger())
        ...
        QShortcut(QKeySequence("Ctrl+Return"), self).activated.connect(self.autocomplete_url)
        QShortcut(QKeySequence("Ctrl+E"), self).activated.connect(self.search_selected_text)
    ...
    def add_new_tab(self, url=None, title="New Tab"):
        ...
        if url is None:
            ...
            url = QUrl()  # Fallback to empty QUrl
        ...
        # Connect signals - fixed urlChanged connection
    ...
    def show_bookmarks(self):
        ...
        if not hasattr(self, 'bookmarks_dialog') or not self.bookmarks_dialog:
            ...
            if not hasattr(self, 'bookmarks_tree') or not self.bookmarks_tree:
                ...
                self.bookmarks_tree.setHeaderLabels(["Name", "URL", "Folder"])
    ...
    def add_current_to_bookmarks(self):
        """Add current page to bookmarks with proper error handling."""
        try:
            browser = self.current_browser()
            if not browser:
                self.notification_manager.show_notification(
                    "Bookmark Error", 
                    "No active browser tab to bookmark",
                    3000
                )
                return
            ...
            if not url or not title:
                self.notification_manager.show_notification(
                    "Bookmark Error", 
                    "Current page has no valid URL or title",
                    3000
                )
                return
            ...
            # Refresh tree if it exists
            if hasattr(self, 'bookmarks_tree') and self.bookmarks_tree:
                self.refresh_bookmarks_tree()
