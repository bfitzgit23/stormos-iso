...
class BrowserCalendar(QWidget):
    ...
    def show_notes_for_date(self, date):
        ...
        if date_str in self.notes:
            raw_text = self.notes[date_str].get('raw', '')
            ...
            <html>
            <body style="color: #888; font-style: ;">
...
class StormBrowserDark(BrowserMainWindow):
    ...
    def show_bookmarks(self, quick_access=False):
        ...
        if quick_access or self.settings_manager.get("quick_bookmark_access", False):
            ...
            searcher = BookmarkSearcher(self)
            searcher.finished.connect(lambda: searcher.search_bar.clear())  # Add this line
            searcher.exec_()
        else:
            ...
            self.bookmarks_dialog.finished.connect(lambda: self.bookmark_search_bar.clear())  # Add this line
    ...
    def _init_bookmark_manager_dialog(self):
        ...
        quick_access_btn.clicked.connect(lambda: [
            self.bookmarks_dialog.close(),
            self.show_bookmarks(quick_access=True)
        ])
        ...
        # Apply dark mode if needed - CORRECTED METHOD NAME
        if self.settings_manager.get("dark_mode"):
