...
class WebPage(QWebEnginePage):
    ...
    def handlePermissionRequest(self, origin, feature):
        ...
        # Grant permission for audio, video and desktop capture (screen share)
        if feature in [
        ...
        (async function() {
            try {
                ...
                for (const device of audioInputs) {
                    if (device.label.toLowerCase().includes('echo cancel') || device.label.toLowerCase().includes('echocancel')) {
                ...
                navigator.mediaDevices.getUserMedia = async function(constraints) {
                    ...
                    if (constraints.audio) {
                        const audioOrig = (typeof constraints.audio === 'boolean') ? {} : constraints.audio;
                        constraints.audio = {
                    ...
                    if (constraints.video) {
                        const videoOrig = (typeof constraints.video === 'boolean') ? {} : constraints.video;
                        constraints.video = {
                            width: { ideal: 1280 },
                            height: { ideal: 720 },
                            frameRate: { ideal: 30 },
                    ...
                    try {
                        ...
                        // Audio processing chain (you can customize here)
                        // [your DSP chain or leave it plain for clarity]

                        // For example, just pass-through without processing:
                ...
                // Add error logging to detect NotReadableError or other getUserMedia issues
                ...
                log('WebRTC constraints injection done');
            } catch (err) {
                try { console.warn('WebRTC injection error:', err); } catch {}
            }
        })();
        """
        try:
            self.runJavaScript(js)
            print("[webrtc_webpage] WebRTC constraints injection executed")
        except Exception as e:
            print("[webrtc_webpage] WebRTC injection error:", e)
...
class BrowserMainWindow(QMainWindow):
    def __init__(self):
        ...
        self.setup_url_launcher_shortcut()  # <-- Add this line
...
# Ensure you have `import logging` and `from PyQt5.QtCore import QRect` at the top of your file
    ...
    def add_new_tab(self, url=None, title="New Tab", background=False, widget=None):
        """
        Adds a new tab to the browser with WebRTC support and echo cancellation.
        Improved with robust focus handling and tab activation.
        ...
        """
        def truncate_title(t, max_len=15):
            """Helper to shorten long tab titles"""
        ...
        # --- PDF Handling ---
        if qurl and qurl.isValid() and self.pdf_viewer.is_pdf_url(qurl):
            return self.pdf_viewer.handle_pdf_request(qurl)
        ...
        # Create browser with WebRTC support
        browser = QWebEngineView()
        ...
        # Set up page with proper permissions
        page = WebPage(profile, browser)
        browser.setPage(page)

        # Set initial URL
        final_url = qurl if qurl and qurl.isValid() else QUrl("about:blank")
        browser.setUrl(final_url)

        # Connect download handler (once per profile)
        ...
        # Ensure tab change handler is connected
        try:
        ...
        # Load favicon if URL is valid
        if final_url.isValid():
            self.load_favicon_for_url(final_url)

        # Connect audio state changes
        page.audioMutedChanged.connect(self.on_audio_state_changed)

        # Inject WebRTC constraints after page loads
        ...
        if not background:
            def activate_new_tab():
                ...
                # 2. Ensure window is active and raised
                self.activateWindow()
                self.raise_()
                
                # 3. Set focus to browser with proper focus chain
                browser.setFocus(Qt.OtherFocusReason)
                
                # 4. Focus URL bar after slight delay
                QTimer.singleShot(10, self.focus_url_bar)

            # Delay activation slightly to ensure UI stability
